...
import TreeLayout.CParseTreeGraph;
 ...
 * @author HP
 */
public class CParseTreeForestGraph extends CParseTreeGraph{
   private CStackNode stackRoot;
  
    /**
     *
     */
    public CParseTreeForestGraph() {
    }
     ...
     * @param root
     */
    public CParseTreeForestGraph(CTreeNode root) {
        super(root);
    }
     ...
     * @param root
     */
    public CParseTreeForestGraph(CStackNode stackRoot, CTreeNode root) {
        super(root);
     ...
     * @return
     */
    public ArrayList<CTreeNode> StatesListtoStackNodesList(ArrayList<Object> aStates){
        ArrayList<CTreeNode> vResult=new ArrayList();
        Object vState;
        for(int i=0;i<aStates.size();i++){
            vState=(int)aStates.get(i);
     ...
     * @return
     */
    public ArrayList<CTreeNode> ParseTreeListtoTreeNodesList(ArrayList<CNode> aTrees){
        ArrayList<CTreeNode> vResult=new ArrayList();
        for(int i=0;i<aTrees.size();i++){
           vResult.add(ParseTreetoTreeNode((CECFGNode)aTrees.get(i)));
        }
        return vResult;
    }
    ...
    if(stackRoot!=null){
       ...
       for(int i=0; i<treeStack.size();i++){
        ...
        //store size information in stackRoot
         ...
         stackRoot.setNodeHeight(vHeight);
         stackRoot.setWidth(Math.max(vNodewidth,vSubtreeAreawidth));
         stackRoot.setHeight(vNodeheight+vEdgeheight+vSubtreeAreaheight);
   ...
   public void draw(Graphics g){
    ...
    if(stackRoot.getStackNodes().size()>1){
       //draw edges,     
       drawEdges(stackRoot,ax,ay,g);
       //draw subtrees
     ...
     * @param g
     */
    public void drawEdges(CStackNode aStackRoot,int ax,int ay,Graphics g){
        stackRoot=aStackRoot;
...
double ax1,ay1, ax2, ay2;	
    ...
    if(stackRoot.getSons().isEmpty()){
        ...
        if(subTreeAlignment == CHorizontalAlignment.BOTTOMALIGNED)
            ay=stackRoot.getNodeHeight()*2+vSpace+stackRoot.getNodeHeight()/2;
        x=hSpace/2;
        y=ay+vSubtree.getY();
        g.drawLine(x,y,x,stackRoot.getHeight()+stackRoot.getSymHeight());
     
    }else{
        ax=stackRoot.getWidth()+hSpace*2;
        x=hSpace+stackRoot.getWidth()+hSpace/2;
        y=ay+vSubtree.getY();
        g.drawLine(x,y,x,stackRoot.getHeight()+stackRoot.getNodeHeight()-stackRoot.getSymHeight());
    } 
