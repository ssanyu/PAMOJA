...
public class CSourceCodeDeterministicParserComp extends CSourceCodeGeneratedParserComp implements ISourceCodeDeterministicParserComp{
    ...
    private String ParserSymCodes=" ";
    private ArrayList<CNTermSet> firstSets;
    /**
   ...
   public void paintComponent(Graphics g) {
     ...
     * @return
     */
    public ArrayList<CNTermSet> getFirstSets() {
        return firstSets;
    }
     ...
     * @param firstSets
     */
    public void setFirstSets(ArrayList<CNTermSet> firstSets) {
     ...
     * @return
     */
    public String getParserSymCodes() {
        return ParserSymCodes;
    }

    /**
     *
     * @param ParserSymCodes
     */
    public void setParserSymCodes(String ParserSymCodes) {
        this.ParserSymCodes = ParserSymCodes;
    }
   ...
   public void setParserSourceStructure(JCompilationUnit aParserSourceStructure){
    ...
    public void setGrammar(IGrammarComp aGrammar){
         ...
         updateParserSource(Grammar.getGrammarStructure());
         updateFirstSets(Grammar.getGrammarStructure());
         //parseText();
         //fire the property change event, with a property that has changed
     ...
     */
    public void updateFirstSets(CGrammar aGrammarStructure){
        CGrammarAnalyzer vAnalyzer=new CGrammarAnalyzer();
         CSE vBody; 
         String vName;
         String sets;
         firstSets=new ArrayList();
         for(int i=0;i<Grammar.getGrammarStructure().getGrammarContext().getNonTerminalDefs().count();i++){
             vName=Grammar.getGrammarStructure().getGrammarContext().getNonTerminalDefs().getElt(i).getName();
             vBody=Grammar.getGrammarStructure().getGrammarContext().getNonTerminalDefs().getElt(i).getBody();
             sets=vAnalyzer.setToString(Grammar.getGrammarStructure(),vAnalyzer.analysisOfECFG(vBody).fFirst);
             firstSets.add(new CNTermSet(vName,sets));
        }   
    }
    /**
     *
    ...
    public void updateParserSource(CGrammar aGrammarStructure){
       ...
       if(!(aGrammarStructure.getStartExpr()instanceof CSE_Empty)){
           ...
           setParserSourceStructure(CompUnitList.getNode(0));
           ParserSymCodes=javaFlatener.f_CompilationUnit(CompUnitList.getNode(1),0);
           // Update textual representation
           ParserSourceText=toText(); 
