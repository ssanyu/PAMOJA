...
public class CGrammarParser extends Parser {
 ...
 /*    public boolean parseSE(){
    ...
    public boolean parseXSE(){
        CSE vSE; // for result
        CSE aSE; // for part
        CSE_List vList; // for element list (of length 1) of stick or dot expression
        ...
        switch ( fScanner.getSym() ){
            case Symbols.syDot:
                ...
                vList = new CSE_List();
                vList.addTerm(aSE);
                vSE = new CSE_MultiDot(vList);
            break;
            case Symbols.syStick:
                nextSym();
                aSE = ps_Id();
                vList = new CSE_List();
                vList.addTerm(aSE);
                vSE = new CSE_MultiStick(vList);
            break;
            default:
                vSE = ps_Expr1();
        }
