...
public class CGrammar extends CTermTuple{
    ...
    public void augmentGrammar(){
      ...
      if(!isAugmented()){
            if(!(fStart instanceof CSE_MultiDot)){
                ...
                //Modify Start Expression to "0.EndMarker"
                vList=new CSE_List();
                vList.add(fStart);
     ...
     * @return
     */
    public String getNameofStartExpr(CSE startExpr){
       String name=new String();
       for(int i=0;i<fSymbolDefs.getNonTerminalDefs().count();i++){
           if(fStart.equals(fSymbolDefs.getNonTerminalDefs().getElt(i)))
               name=fSymbolDefs.getNonTerminalDefs().getElt(i).getName();
       }
       return name;
   }
