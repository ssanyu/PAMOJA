...
public class CSymbolStream implements ISymbolStream {
    ...
    public void previousSym(){
            ...
            if(fPosition.Col==0 && fPosition.Line>0){
                fPosition.Line--;
                fPosition.Col=fSymbols.get(fPosition.Line).size()-1;
            }else if(fPosition.Col>0 && fPosition.Line==0){
                 fPosition.Col--;
     ...
     * @param aSymbols
     */
    public void addAll(ArrayList<ArrayList<Symbol>> aSymbols){
        fSymbols=new ArrayList<>();
        for(int i=0;i<aSymbols.size();i++){
            fSymbols.add(aSymbols.get(i));
        }
   ...
   /**
    * Returns a sequence of symbol values.
    * @return the sequence of symbol values.
    */ 
    @Override
   public String toString(){
     ...
     * @return
     */
    public ArrayList<Symbol> getSymbolnamevalue(){
        String vSymName;
        String vSymValue;
        ArrayList<Symbol> vResult=new ArrayList();
        for( int i=0; i<fSymbols.size();i++){
               for(int j=0;j<fSymbols.get(i).size();j++){
                 vSymName=fSymbols.get(i).get(j).symName();
                 vSymValue=fSymbols.get(i).get(j).symValue();
                 vResult.add(new Symbol(vSymName,vSymValue)); 
                }
            }
